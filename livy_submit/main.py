...
def _make_arg_parser():
    ...
    ap.add_argument(
        '--hdfs-dir',
        action='store',
        help=('Directory on hdfs to store input files for spark job. Defaults to '
    ...
    ap.add_argument(
        ...
        required=True,
        action='store',
        help=("The FQDN of the namenode where webHDFS is running. Optionally include the port "
    ...
    ap.add_argument(
        ...
        required=True,
        action='store',
        help=("The livy hostname and port. e.g. --livy-server "
...
def _parse_config(config: list) -> dict:
    ...
    for kv in config:
        try:
            ...
            logging.error(f"Config values must be in the form key=value. You passed in {kv}")
            raise
        cfg[k] = v
    return cfg
...
def cli():
    ...
    run(hdfs_dir, spark_config, run_file, namenode_connection_url, livy_connection_url, job_name)


def run(hdfs_dir, spark_config, run_file, namenode_connection_url, livy_connection_url,
        job_name):
    ...
    last_stop = 0
    for i in range(100):
        r = requests.get(connection_url + f"/{submission_id}/log", headers=headers, auth=auth)
        r.raise_for_status()
        logs_json = r.json()
